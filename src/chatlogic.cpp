...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // use smart pointer for GraphNode
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // Use unique pointer as a parameter
                            ...
                            // get pointer from smart pointer
                            ...
                            // use smartpointer function & move semantics
    ...
    // identify root node
    // use smart pointer for root node
    GraphNode *rootNode = nullptr;
    // auto rootNode = std::make_unique<GraphNode>(nullptr);
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                ...
                rootNode = it->get(); // assign current node to root from smart pointer
            }
            else
            {
